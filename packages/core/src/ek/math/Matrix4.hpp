...
#include <ek/math/internal/compiler_unsafe_end.h>
    ...
    inline explicit Matrix(T v = 1) {
        set_identity(v);
    }

    inline Matrix(T m00_, T m01_, T m02_, T m03_,
    ...
    inline Matrix(const vec4& v0, const vec4& v1, const vec4& v2, const vec4& v3) noexcept {
        (*this)(0, 0) = v0.x;
        (*this)(0, 1) = v0.y;
        (*this)(0, 2) = v0.z;
        (*this)(0, 3) = v0.w;
    ...
    inline Matrix(const mat4x4& matrix) noexcept {
        for (int i = 0; i < 16; ++i) {
            m[i] = matrix.m[i];
        }
    }

    inline Matrix(mat4x4&& matrix) noexcept = default;
    ...
    void setTransform2D(Vec2<T> position, T scale = 1, T rotation_degrees = 0) {
...
template<typename T>
Matrix<4, 4, T> operator*(const Matrix<4, 4, T>& m, const T f) {
    Matrix<4, 4, T> r;
    for (int i = 0; i < 16; ++i) {
        r.m[i] = m.m[i] * f;
    }
...
template<typename T>
inline Matrix<4, 4, T> operator*(const Matrix<4, 4, T>& l, const Matrix<4, 4, T>& r) {
    return Matrix<4, 4, T>{
...
template<typename T>
Matrix<4, 4, T> operator*(const Matrix<4, 4, T>& a, const Matrix<3, 2, T>& b) {
...
template<typename T>
Matrix<3, 3, T>::Matrix(const Matrix<4, 4, T>& mat) noexcept
