...
#include <ek/math/internal/compiler_unsafe_end.h>
    ...
    Quaternion(T w_, T x_, T y_, T z_) noexcept:
            x{x_},
            y{y_},
            z{z_},
            w{w_} {
    ...
    explicit Quaternion(const Vec3 <T>& euler_angles) {
        const Vec3<T> c{
    ...
    Quaternion(T w_, const Vec<3, T>& v) : w{w_}, x{v.x}, y{v.y}, z{v.z} {}

    explicit Quaternion(const Matrix<4, 4, T>& m) {
    ...
    inline Vec<3, T> operator*(const Vec<3, T>& v) const {
        const Vec<3, T> qua_vector{x, y, z};
        const Vec<3, T> uv(cross(qua_vector, v));
        const Vec<3, T> uuv(cross(qua_vector, uv));
...
template<typename T>
Vec3 <T> euler_angles(const Quaternion<T>& q) {
...
template<>
inline Quaternion<float> normalize(const Quaternion<float>& a) {
    return a * Math::Q_rsqrt(a.x * a.x + a.y * a.y + a.z * a.z + a.w * a.w);
}

template<typename T>
Quaternion<T> quat_cast(const Matrix<3, 3, T>& m) {
    ...
    switch (biggest_index) {
        case 0:
            return Quaternion<T>(biggest_val,
                                 (m(1, 2) - m(2, 1)) * mult,
                                 (m(2, 0) - m(0, 2)) * mult,
                                 (m(0, 1) - m(1, 0)) * mult);
        case 1:
            return Quaternion<T>((m(1, 2) - m(2, 1)) * mult, biggest_val,
                                 (m(0, 1) + m(1, 0)) * mult,
                                 (m(2, 0) + m(0, 2)) * mult);
        case 2:
            return Quaternion<T>((m(2, 0) - m(0, 2)) * mult,
                                 (m(0, 1) + m(1, 0)) * mult, biggest_val,
                                 (m(1, 2) + m(2, 1)) * mult);
        case 3:
            return Quaternion<T>((m(0, 1) - m(1, 0)) * mult,
                                 (m(2, 0) + m(0, 2)) * mult,
                                 (m(1, 2) + m(2, 1)) * mult,
                                 biggest_val);
        default:
...
template<typename T>
Quaternion<float> quat_between(const Vec3 <T>& u, const Vec3 <T>& v) {
    ...
    if (real_part < static_cast<T>(1.e-6f) * norm_u_norm_v) {
        ...
        t = std::abs(u.x) > std::abs(u.z) ? Vec3<T>(-u.y, u.x, static_cast<T>(0)) : Vec3<T>(static_cast<T>(0), -u.z,
                                                                                            u.y);
    } else {
    ...
    return normalize(Quaternion<float>(real_part, t.x, t.y, t.z));
}

template<typename T>
Quaternion<T> quat_look_at_rh(const Vec<3, T>& direction, const Vec<3, T>& up) {
    Vec<3, T> u2 = -direction;
    Vec<3, T> u0 = normalize(cross(up, u2));
    Vec<3, T> u1 = cross(u2, u0);

    Matrix<3, 3, T> m{u0, u1, u2};
