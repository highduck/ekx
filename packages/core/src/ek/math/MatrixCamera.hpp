...
// perspectiveRH_ZO
template<typename T>
Matrix<4, 4, T> perspective_rh(T fov_y, T aspect, T z_near, T z_far) {
...
// perspectiveLH_ZO
template<typename T>
Matrix<4, 4, T> perspective_lh(T fov_y, T aspect, T z_near, T z_far) {
    const T tan_half_fov_y = std::tan(fov_y / static_cast<T>(2));
    Matrix<4, 4, T> m{0};
    m(0, 0) = static_cast<T>(1) / (aspect * tan_half_fov_y);
    m(1, 1) = static_cast<T>(1) / tan_half_fov_y;
...
// orthoLH_ZO
template<typename T>
Matrix4 <T> ortho_projection_lh(T left, T right, T bottom, T top, T z_near, T z_far) {
    Matrix4<T> m{};
...
// orthoRH_ZO
template<typename T>
Matrix4 <T> ortho_projection_rh(T left, T right, T bottom, T top, T z_near, T z_far) {
    Matrix4<T> m{};
    m(0, 0) = static_cast<T>(2) / (right - left);
    m(1, 1) = static_cast<T>(2) / (top - bottom);
    m(2, 2) = -static_cast<T>(1) / (z_far - z_near);
...
template<typename T>
Matrix4 <T> ortho_2d(T x, T y, T width, T height, T z_near = -1, T z_far = 1) {
    EK_ASSERT_R2(width != 0 && height != 0);
    return ortho_projection_rh(x, x + width, y + height, y, z_near, z_far);
}

// TODO:  look at view matrix calc
template<typename T>
Matrix<4, 4, T> look_at_rh(const Vec<3, T>& eye, const Vec<3, T>& center, const Vec<3, T>& up) {
...
template<typename T>
Matrix<4, 4, T> look_at_lh(const Vec<3, T>& eye, const Vec<3, T>& center, const Vec<3, T>& up) {
    const Vec<3, T> f(normalize(center - eye));
