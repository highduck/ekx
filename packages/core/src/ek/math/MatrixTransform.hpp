...
template<typename T>
Matrix<4, 4, T> rotate(const Matrix<4, 4, T>& m, T angle, const Vec3 <T>& v) {
    T const a = angle;
    T const c = cos(a);
    T const s = sin(a);
...
// X-Z-Y
template<typename T>
inline Matrix<4, 4, T> rotation_transform(const Vec3 <T>& v) {
    T c1 = cos(-v.x);
    T c2 = cos(-v.y);
    T c3 = cos(-v.z);
    T s1 = sin(-v.x);
...
template<typename T>
inline Matrix<4, 4, T> rotation_transform(const Quaternion <T>& q) {
    T qxx = q.x * q.x;
    T qyy = q.y * q.y;
    T qzz = q.z * q.z;
    T qxz = q.x * q.z;
...
template<typename T>
Matrix<4, 4, T> translate_transform(const Vec<3, T>& translation) {
    Matrix<4, 4, T> m{};
    m(3, 0) = translation.x;
    m(3, 1) = translation.y;
    m(3, 2) = translation.z;
...
template<typename T>
Matrix<4, 4, T> scale_transform(const Vec<3, T>& scale) {
    Matrix<4, 4, T> m{};
    m(0, 0) = scale.x;
    m(1, 1) = scale.y;
    m(2, 2) = scale.z;
...
template<typename T>
Vec<3, T> extract_translation(const Matrix<4, 4, T>& m) {
    return Vec<3, T>{m(3, 0), m(3, 1), m(3, 2)};
}
