...
import {cpus, platform} from "https://deno.land/std/node/os.ts";
import {rm, getModuleDir} from "./utils.ts";
...
export function resolveOptions(options?: BuildOptions): BuildOptions {
    ...
    if (opts.ccache && opts.os !== platform()) {
        if (opts.os !== "web") {
            opts.ccache = false;
        }
    }
    ...
    switch (opts.os) {
        ...
        case "ios":
            if (!opts.toolchain) {
                opts.toolchain = path.resolve(getModuleDir(import.meta), "cmake/ios.cmake");
            }
        ...
        case "windows":
            if (platform() === "darwin") {
                if (!opts.toolchain) {
                    opts.toolchain = path.resolve(getModuleDir(import.meta), "cmake/mingw-w64-x86_64.cmake");
...
export async function buildMatrix(options?: BuildOptions & BuildMatrix): Promise<void> {
    ...
    const osList = getOptionalMany(options?.os, [platform()]);
    if (options?.parallel) {
