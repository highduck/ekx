/*
 * modified version of `miniz-cpp` library
...
#define MINIZ_CPP_THROW(x) abort()
...
public:
    ...
    std::string read(const zip_info& info) {
        std::size_t size;
        char* data = static_cast<char*>(mz_zip_reader_extract_file_to_heap(archive_.get(), info.filename.c_str(), &size,
                                                                           0));
        if (data == nullptr) {
            MINIZ_CPP_THROW("file couldn't be read");
        }
    ...
    void remove_comment() {
        ...
        for (; position > 3; position--) {
            if (buffer_[position - 3] == 'P'
                ...
                if (buffer_[position - 1] == '\x05'
                    && buffer_[position] == '\x06') {
                    position = position + 17;
                    break;
                }
//                else if(buffer_[position - 1] == '\x03'
//                        && buffer_[position] == '\x04') {
...
//                }
        ...
        if (position == 3) {
            MINIZ_CPP_THROW("didn't find end of central directory signature");
        }

        uint16_t length = static_cast<uint32_t>(buffer_[position + 1]);
