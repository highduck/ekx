...
- (void)ad:(id)ad didFailToPresentFullScreenContentWithError:(NSError *)error {
    ...
    admob::context.onEvent(admob::EventType::VideoFailed);
    //admob::reloadRewardedAd();
}

- (void)adDidDismissFullScreenContent:(id)ad {
    NSLog(@"Rewarded ad dismissed.");
    admob::context.onEvent(admob::EventType::VideoClosed);
}
...
void reloadRewardedAd() {
    ...
    completionHandler: ^(GADRewardedAd *ad, NSError *error) {
        if (error) {
            ...
            context.onEvent(EventType::VideoLoaded);
        }
    }];
}
...
void show_rewarded_ad() {
    ...
    if (isRewardedAdReady()) {
        ...
        userDidEarnRewardHandler:^ {
            ...
            context.onEvent(EventType::VideoRewarded);
        }];
    } else {
        NSLog(@"Ad wasn't ready");
        reloadRewardedAd();
        context.onEvent(EventType::VideoLoading);
    }
}
