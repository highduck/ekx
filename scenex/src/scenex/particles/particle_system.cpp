...
void particles_burst(ecs::entity e, int count) {
    ...
    ek::asset_t<particle_decl> decl{emitter.particle};
    auto& layer = find_particle_layer(e);
    while (count > 0) {
        auto* p = produce_particle(*decl);
        add_particle(layer, p);
        float2 pos = position;
        pos.x += ek::random(data.rect.x, data.rect.right());
        pos.y += ek::random(data.rect.y, data.rect.bottom());
        p->position = pos;
    ...
    for (auto e : ecs::view<particle_emitter_t>()) {
        ...
        if (emitter.time >= data.interval) {
            ...
            while (count > 0) {
                ...
                x += data.offset.x;
                y += data.offset.y;
                x += ek::random(data.rect.x, data.rect.right());
                y += ek::random(data.rect.y, data.rect.bottom());
...
particle* spawn_particle(ecs::entity e, const std::string& particle_id) {
    ek::asset_t<particle_decl> decl{particle_id};
    if (decl) {
        auto* p = produce_particle(*decl);
