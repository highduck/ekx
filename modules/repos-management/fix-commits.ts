...
'`;
    await execute("git", ["filter-branch", envScript, "--tag-name-filter", "cat", "--", "--branches", "--tags"], {workingDir});
    await execute("git", ["push", "origin", "--force", "--all"], {workingDir});
    await execute("git", ["push", "origin", "--force", "--tags"], {workingDir});
};
