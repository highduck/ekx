...
export async function buildDev(ctx: Project): Promise<any> {
    ...
    const materialIconsFont = resolveFrom(ctx.sdk.EKX_ROOT, "@ekx/dev-fonts/ttf/MaterialIcons-Regular.ttf");
    const awesomeIconsFont = resolveFrom(ctx.sdk.EKX_ROOT, "@ekx/dev-fonts/ttf/fa-solid-900.ttf");
    ...
    fs.copyFileSync(path.join(ctx.sdk.EKX_ROOT, "dev-tools/assets/sf-pro-text-regular.ttf"), path.join(exportDir, "dev/sf-pro-text-regular.ttf"));
    fs.copyFileSync(path.join(ctx.sdk.EKX_ROOT, "dev-tools/assets/sf-mono-text-regular.ttf"), path.join(exportDir, "dev/sf-mono-text-regular.ttf"));
}