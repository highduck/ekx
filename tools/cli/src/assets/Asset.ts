import {isDir, isFile, makeDirs} from "../utils";
...
export class AssetBuilderContext {
    ...
    populate() {
        try {
            ...
            const scripts = glob.sync(path.join(bp, "**/assets.js"));
    ...
    async buildIfChanged() {
        ...
        if (isFile(inputsHashFilepath)) {
            const hash = fs.readFileSync(inputsHashFilepath, "utf8");
        ...
        fs.writeFileSync(inputsHashFilepath, inputsHashString, "utf8");
    }
