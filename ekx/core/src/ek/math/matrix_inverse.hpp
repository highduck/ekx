...
template<typename T>
matrix_t<2, 2, T> affine_inverse(matrix_t<2, 2, T> m) {
    T d = det(m);
...
template<typename T>
matrix_t<3, 3, T> inverse(matrix_t<3, 3, T> m) {
...
template<typename T>
matrix_t<4, 4, T> inverse(matrix_t<4, 4, T> m) {
    ...
    vec4 sign_a(+1, -1, +1, -1);
    vec4 sign_b(-1, +1, -1, +1);
    matrix_t<4, 4, T> inv_m(inv_0 * sign_a, inv_1 * sign_b, inv_2 * sign_a, inv_3 * sign_b);
